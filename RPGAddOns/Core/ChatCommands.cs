...
namespace RPGAddOns.Core
{
    ...
    {
        [Command(name: "hudtest", shortHand: "hud", adminOnly: false, usage: ".rpg pr", description: "")]
        public static void canvasTesting(ChatCommandContext ctx)
        {
            // start with resources find
            Entity playerCharacter = ctx.Event.SenderCharacterEntity;
            EntityManager entityManager = VWorld.Server.EntityManager;
            Il2CppSystem.Type canvasType = Il2CppSystem.Type.GetType("ProjectM.UI.UICanvasBase, ProjectM");
            var canvasEntry = Resources.FindObjectsOfTypeAll(canvasType);
            // check for entries
            for (int i = 0; i < canvasEntry.Length; i++)
            {
                Plugin.Logger.LogInfo($"Entry {i}: {canvasEntry[i].name}");
                GameObject canvasBase = (GameObject)UnityEngine.Object.Instantiate(canvasEntry[i]);
                Plugin.Logger.LogInfo($"Instantiation successfull");
                //UICanvasBase uICanvasBase = canvas as UICanvasBase;
                // want highest parent, should be HUDCanvas

                GameObject HUDCanvas = canvasBase.transform.root.gameObject;
                Plugin.Logger.LogInfo($"{HUDCanvas.gameObject.");
                HUDCanvas.name = "UICanvas";
            }
        }
