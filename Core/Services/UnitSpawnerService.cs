...
internal class UnitSpawnerService
{
    ...
    {
        var translation = Utilities.GetComponentData<Translation>(user);
        var f3pos = new float3(position.x, translation.Value.y, position.y); // TODO: investigate this copypasta
        UnitSpawnerUpdateSystem unitSpawnerUpdateSystem = VWorld.Server.GetExistingSystem<UnitSpawnerUpdateSystem>();

        UnitSpawnerReactSystem_Patch.Enabled = true;

        var durationKey = NextKey();
        unitSpawnerUpdateSystem.SpawnUnit(empty_entity, unit, f3pos, 1, DEFAULT_MINRANGE, DEFAULT_MAXRANGE, durationKey);
    ...
    public static class UnitSpawnerReactSystem_Patch
    {
        ...
        public static void Prefix(UnitSpawnerReactSystem __instance)
        {
            ...
            foreach (var entity in entities)
            {
                ...
                if (!Utilities.HasComponent<LifeTime>(entity)) return;

                var lifetimeComp = Utilities.GetComponentData<LifeTime>(entity);
                ...
                if (UnitSpawner.PostActions.TryGetValue(durationKey, out var unitData))
                {
                    ...
                    Utilities.SetComponentData(entity, newLifeTime);
