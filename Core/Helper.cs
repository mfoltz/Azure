...
public static class Helper
{
    ...
    public static void CreateJewel(FromCharacter fromData, string spellName, string mods = "", float power = 1f)
    {
        PrefabGUID abilityGuid = JewelData.abilityToPrefabDictionary[spellName];
        if (!jewelSpawnSystem.TryCreateJewelAndAddToInventory(fromData.Character, abilityGuid, 2, out var jewelEntity))
        {
            return;
        }
        ...
        power = (float)(0.15 + (double)power * 0.85);
        SpellModSetComponent componentData = jewelEntity.Read<SpellModSetComponent>();
        if (mods == "")
        {
            for (int i = 0; i < JewelData.SpellMods[spellName].Count && i < 8; i++)
            {
                mods += i + 1;
            }
        }

        int index = int.Parse(mods[0].ToString()) - 1;
        int index2 = int.Parse(mods[1].ToString()) - 1;
        int index3 = int.Parse(mods[2].ToString()) - 1;
        componentData.SpellMods.Mod0.Id = JewelData.SpellMods[spellName][index].Key;
        if (flag)
        {
            power = (float)random.NextDouble();
            power = 0.15f + power * 0.85f;
        }

        componentData.SpellMods.Mod0.Power = power;
        componentData.SpellMods.Mod1.Id = JewelData.SpellMods[spellName][index2].Key;
        if (flag)
        {
            power = (float)random.NextDouble();
            power = 0.15f + power * 0.85f;
        ...
        componentData.SpellMods.Mod1.Power = power;
        componentData.SpellMods.Mod2.Id = JewelData.SpellMods[spellName][index3].Key;
        if (flag)
        {
            power = (float)random.NextDouble();
            power = 0.15f + power * 0.85f;
        ...
        componentData.SpellMods.Mod2.Power = power;
        if (mods.Length > 3)
        {
            int index4 = int.Parse(mods[3].ToString()) - 1;
            componentData.SpellMods.Mod3.Id = JewelData.SpellMods[spellName][index4].Key;
            if (flag)
            {
                power = (float)random.NextDouble();
                power = 0.15f + power * 0.85f;
            ...
            componentData.SpellMods.Mod3.Power = power;
            if (mods.Length > 4)
            {
                int index5 = int.Parse(mods[4].ToString()) - 1;
                componentData.SpellMods.Mod4.Id = JewelData.SpellMods[spellName][index5].Key;
                if (flag)
                {
                    power = (float)random.NextDouble();
                    power = 0.15f + power * 0.85f;
                ...
                componentData.SpellMods.Mod4.Power = power;
                if (mods.Length > 5)
                {
                    int index6 = int.Parse(mods[5].ToString()) - 1;
                    componentData.SpellMods.Mod5.Id = JewelData.SpellMods[spellName][index6].Key;
                    if (flag)
                    {
                        power = (float)random.NextDouble();
                        power = 0.15f + power * 0.85f;
                    ...
                    componentData.SpellMods.Mod5.Power = power;
                    if (mods.Length > 6)
                    {
                        int index7 = int.Parse(mods[6].ToString()) - 1;
                        componentData.SpellMods.Mod6.Id = JewelData.SpellMods[spellName][index7].Key;
                        if (flag)
                        {
                            power = (float)random.NextDouble();
                            power = 0.15f + power * 0.85f;
                        ...
                        componentData.SpellMods.Mod6.Power = power;
                        if (mods.Length > 7)
                        {
                            int index8 = int.Parse(mods[7].ToString()) - 1;
                            componentData.SpellMods.Mod7.Id = JewelData.SpellMods[spellName][index8].Key;
                            if (flag)
                            {
                                power = (float)random.NextDouble();
                                power = 0.15f + power * 0.85f;
                            ...
                            componentData.SpellMods.Mod7.Power = power;
                        }
                    }
                }
            }
        ...
        jewelEntity.Write(componentData);
    }
